...
fn clear_display(fb: &mut OrbicFramebuffer) {
    fb.write_buffer(&vec![(0, 0, 0); 128 * 128]);
}
...
fn display_qr_code(fb: &mut OrbicFramebuffer, qr_text: &str) {
    ...
    if let Ok(code) = QrCode::with_error_correction_level(qr_text, qrcode::EcLevel::L) {
        let image = code.render::<Luma<u8>>().max_dimensions(123, 123).build();
        let mut buffer = vec![(0, 0, 0); 128 * 128];
        for y in 0..image.height().min(123) {
            for x in 0..image.width().min(123) {
                let pixel = image.get_pixel(x, y)[0];
                let color = if pixel == 0 { (0, 0, 0) } else { (255, 255, 255) };
                let idx = (y + 2) as usize * 128 + (x + 2) as usize;
                buffer[idx] = color;
            }
        }
